...
def extract_details(filename):
    ...
    player_details = pd.DataFrame(columns=['date','team','runs','balls','not_out','opposition','batting_order','venue','outcome'])
    ...
    for ball in innings1:
        for delivery in ball:
            ...
            player_details.loc[ball[delivery]['batsman'], 'oppostion'] = match_details['team'+str(match_details['bat_first'][1])]
            player_details.loc[ball[delivery]['batsman'], 'batting_order'] = 1
    ...
    try:
        ...
        for ball in innings2:
            for delivery in ball:
                ...
                player_details.loc[ball[delivery]['batsman'], 'team'] = match_details['team'+str(match_details['bat_first'][1])]
                player_details.loc[ball[delivery]['batsman'], 'opposition'] = match_details['bat_first']
                player_details.loc[ball[delivery]['batsman'], 'batting_order'] = 2
                player_details.loc[ball[delivery]['batsman'], 'venue'] = match_details['venue']
                if match_details['winner'] == player_details.loc[ball[delivery]['batsman'], 'team']:
                    player_details.loc[ball[delivery]['batsman'], 'outcome'] = 1
                else:
...
overall_batsman_details = pd.DataFrame(columns=['team','innings', 'average', 'strike_rate','fifties','centuries','zeros','runs','balls'])
match_batsman_details = pd.DataFrame(columns=['date','name','team','opposition','venue','batting_order','outcome','runs','balls','not_out','strike_rate'])
...
for filename in os.listdir('odis'):
    if filename.endswith('.yaml'):
        ...
        for player in player_details.index:
            ...
            overall_batsman_details.loc[player,'strike_rate'] += overall_batsman_details.loc[player,'runs']/overall_batsman_details.loc[player,'balls']
            if overall_batsman_details.loc[player,'runs'] == 0:
                overall_batsman_details.loc[player,'zeros'] += 1
            elif overall_batsman_details.loc[player,'runs']>=50 and overall_batsman_details.loc[player,'runs']<100:
                overall_batsman_details.loc[player,'fifties'] += 1
            elif overall_batsman_details.loc[player,'runs']>=100:
            ...
            match_batsman_details.loc[count,'batting_order'] = player_details.loc[player,'batting_order']
            match_batsman_details.loc[count,'outcome'] = player_details.loc[player,'outcome']
            ...
            match_batsman_details.loc[count,'not_out'] = player_details.loc[player,'not_out']
            match_batsman_details.loc[count,'strike_rate'] = player_details.loc[player,'runs']/player_details.loc[player,'balls']

overall_batsman_details['average'] = overall_batsman_details['runs']/overall_batsman_details['innings']
overall_batsman_details['strike_rate'] /= overall_batsman_details['innings']

match_batsman_details.set_index(keys=['date','name'],drop=True,inplace=True)
